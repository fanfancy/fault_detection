...
linenum_all = ['316', '75', '72', '69', '66','81', '319']
...
if net_mode == 1:
    ...
    from cnn_model_2branch import *
    layers = [length,3,1,length//2,3,n_imfs*2]
...
for linenum in linenum_all:
    ...
    # HHT = np.zeros((kinds, length//2, 3, n_imfs))
    HHT = np.zeros((kinds, length//2, 3, n_imfs*2))
    for i in range(n_imfs):
        for j in range(3):
            HHT[:,:,j,i] = data2[:,start:end,2*i+1,j]
            # HHT[:,:,j,i] = data2[:,:,i,j]
            
    print('HHT.shape',HHT.shape)
    ...
    HHT_type_train = HHT_type[:int(kinds*0.75)]
    HHT_type_test = HHT_type[int(kinds*0.75):]
    ...
    print ("HHT_type_train.shape",HHT_type_train.shape)
    print ("HHT_type_test.shape",HHT_type_test.shape)
    ...
    if net_mode==1:
        ...
        input_data = {'main_input':data3_type,'hht_input':HHT_type}
        data_train = {'main_input':data3_type_train,'hht_input':HHT_type_train}
        data_test = {'main_input':data3_type_test,'hht_input':HHT_type_test}
    ...
    if faulttype == 1:
        ...
        if record:
            with open("0308_rand300_log_netmode6_2branch.txt",'a+') as f:
                f.write("@ train %s.\n" % (linenum))
        ...
        ##  with open("0308_rand300_log_netmode6_2branch_error_type.txt",'w') as f:
