...
def plot_imfs(savepath,signal, imfs, time_samples=None, fignum=None, show=True):
    """
    ...
    for i in range(n_imfs - 1):
        ...
        if is_bivariate:
            ...
            print ("not is_bivariate")
            ax.plot(time_samples, (imfs[i]))
        #ax.axis([time_samples[0], time_samples[-1], -axis_extent, axis_extent])
        #ax.tick_params(which='both', left=True, bottom=True, labelleft=True,
        #               labelbottom=True)
        ax.grid(True)
    #    ax.set_ylabel('imf' + str(i + 1))
    '''
    # Plot the residue
    ax = plt.subplot(1,n_imfs + 1,  n_imfs + 1)
    if is_bivariate:
