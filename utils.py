...
# ___________________judge stop while or not ____________________#
def judge_conditions(signal, ts):
    tmax = argrelmax(signal)[0]
    maxima = signal[tmax]
    tmin = argrelmin(signal)[0]
    minima = signal[tmin]
    # tmin, tmax, xmin, xmax = boundary_conditions(signal, ts)
    if len(tmax) > 3 and len(tmin) > 3:
        return True

    else:
        return False
    
def judge_stop(signal):
    tmax = argrelmax(signal)[0]
    maxima = signal[tmax]
    tmin = argrelmin(signal)[0]
    minima = signal[tmin]
