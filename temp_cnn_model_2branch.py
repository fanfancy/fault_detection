...
def build_model_type1(input,fault_type):
    ...
    x = MaxPooling2D(pool_size=(4, 1))(x)					                        #100*3*4
    x = Conv2D(8, (8, 1), activation='relu', padding='same')(x)			            #100*3*8
    x = MaxPooling2D(pool_size=(4, 1))(x)					                        #25*3*8
    x = Conv2D(16, (8, 1), activation='relu',padding='same')(x) 			        #25*3*16
    x = MaxPooling2D(pool_size=(4, 1))(x)					                        #6*3*16
    x = Conv2D(32, (8, 1), activation='relu',padding='same')(x) 			        #6*3*32
