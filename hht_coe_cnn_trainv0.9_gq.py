...
#linenum_all = ['49']
#linenum_all = ['75']
linenum_all = ['49', '269', '313', '316', '75', '72', '69', '66', '81', '319']
#linenum_all = ['81', '319']
# linenum_all = ['313']
...
if net_mode == 1:
    ...
    layers = [length,3,1,length,3,n_imfs*2]
...
for linenum in linenum_all:
    ...
    for i in range(kinds):
        data2[i, :, :, :] = res[i, :, 2: n_imfs * 2+2, :]
    print ('HHT_ori',data2.shape)
    ...
    HHT = np.zeros((kinds, length, 3, n_imfs*2))
    for i in range(n_imfs):
        for j in range(3):
            HHT[:,:,j,i] = data2[:,start:end,2*i+1,j]
            # HHT[:,:,j,i] = data2[:,:,i,j]
            
    print('HHT_f',HHT.shape)
    ...
    if faulttype == 1:
        ...
        checkpoint = ModelCheckpoint(log_dir + linenum +"_faulttype.h5",
                            monitor="val_loss",
                            mode='min',
        ...
        if record:
            with open("rand300_log_htt0303_gq.txt",'a+') as f:
                f.write("@ train %s.\n" % (linenum))
...
############### Phase test ########################2
    if faultphase == 1:
        ...
        #记录所有训练过程，每隔一定步数记录最大值
        tensorboard = TensorBoard(log_dir=log_dir)
        checkpoint = ModelCheckpoint(log_dir + linenum +"_faultphase.h5",
                            monitor="val_loss",
                            mode='min',
                            save_weights_only=True,
                            save_best_only=True, 
    ...
    if faultloc == 1:
        ...
        #记录所有训练过程，每隔一定步数记录最大值
        tensorboard = TensorBoard(log_dir=log_dir)
        checkpoint = ModelCheckpoint(log_dir + linenum +"_faultloc.h5",
                            monitor="val_loss",
                            mode='min',
                            save_weights_only=True,
                            save_best_only=True, 
