...
if net_mode == 1:
    ...
    from htt_coe_cnn_model_coe_only import *
    layers = [length,3,1,n_imfs]
elif net_mode == 5:
...
for linenum in linenum_all:
    ...
    if net_mode==1:
        ...
        input_data = {'hht_co_input':HHT_coefficients_type}
        data_train = {'hht_co_input':HHT_coe_train}
        data_test = {'hht_co_input':HHT_coe_test}
    elif net_mode==5:
        input_data = {'main_input':data2_type,'hht_co_input':HHT_coefficients_type}
    ...
    if faulttype == 1:
        ...
        if record:
            with open("0309_coe_only.txt",'a+') as f:
                f.write("@ train %s.\n" % (linenum))
        ...
        ##  with open("0309_coe_only_error_type.txt",'w') as f:
